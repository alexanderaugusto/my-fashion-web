...
import { InputAddon, Button, CardDescription } from "../../components"
...
export default class AddressEditForm extends Component {
  constructor(props) {
    ...
    this.state = {
      addressInfo: {
        ...
        streetState: "has-success",
        neighborhoodState: "has-success",
        numberState: "has-success",
        complementState: "has-success",
  ...
  getAddressInfo() {
    ...
    const { zipcode, city, state, street, neighborhood, number, complement, name, phone, is_main: isMain } = this.props.address
    ...
    addressInfo.number = number
    addressInfo.complement = complement
    addressInfo.name = name
    addressInfo.phone = phone
    addressInfo.isMain = isMain

    this.setState({ addressInfo, isMain })
  }
  ...
  edit() {
    ...
    const { zipcode, city, state, street, neighborhood, number, complement, name, phone,
      isMain: is_main } = this.state.addressInfo
    const data = { id, zipcode, city, state, street, neighborhood, number, complement, name, phone, is_main }
    ...
    return (
      <div className="text-center">
        <Form onSubmit={(e) => this.checkData(e)}>
          ...
          {this.state.isMain ? null :
            <div className="text-left check-box">
              <FormGroup check>
                <Label check>
                  <Input type="checkbox" onChange={e => {
                    const addressInfo = this.state.addressInfo
                    addressInfo.isMain = !addressInfo.isMain
                    this.setState({ addressInfo })
                  }} />
                  <span className="form-check-sign" />
                  <CardDescription fontSize={18}> Tornar esse endereço principal </CardDescription>
                </Label>
              </FormGroup>
            </div>}
